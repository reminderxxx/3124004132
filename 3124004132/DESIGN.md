...
`calculate_similarity()` 作为查重模块的统一入口，依次完成文本规范化、特征提取、覆盖率和余弦相似度计算。完全相同或规范化后等价的文本重复率为 `1.0`；局部增删改会根据共享内容和顺序变化得到中间值。
...
本阶段先确定输入输出约定、模块职责和可替换的算法接口。文本规范化、空文本处理、多粒度字符特征、覆盖率和余弦相似度已经实现。当前组合方式能够反映内容增删和局部调序，后续还需要通过覆盖率检查和性能分析确认边界情况。
