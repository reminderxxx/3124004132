...
`extract_features()` 将把文本转换为带频次的特征集合。具体采用的字符窗口大小和两类特征的组合方式，会在实现与测试阶段通过对比确定。
...
本阶段先确定输入输出约定、模块职责和可替换的算法接口。文本规范化和空文本处理规则已经实现；字符窗口大小、词语特征提取方式以及不同特征的组合比例，将在后续编码和测试阶段分别比较后确定，并记录选择依据。
