...
1. 使用 Unicode 规范化统一全角、半角等字符形式。
2. 统一换行、制表符和多余空白。
3. 在特征计算时降低标点差异带来的影响。
4. 对少量含义稳定的常见等价表达进行统一，例如将“星期日”“星期天”“周日”和“周天”映射为同一种表达。
...
本阶段先确定输入输出约定、模块职责和可替换的算法接口。字符窗口大小、词语特征提取方式、不同特征的组合比例以及空文本处理规则，将在编码和测试阶段分别比较后确定，并记录选择依据。
